                              ...
                              char * keyValue, size_t keyValueSize)
{
  ...
  if ((strlen(keyName) > CONF_MAX_LINE_LENGTH)
      || (strlen(inputStr) > CONF_MAX_LINE_LENGTH))
    return 0;
  ...
  if ((tempKeyNameLen > CONF_MAX_LINE_LENGTH)
      || (tempKeyValueLen > CONF_MAX_LINE_LENGTH))
    return 0;
                     ...
                     char * keyValue, size_t keyValueSize)
{
  ...
  if (strlen(fileName) > CONF_MAX_PATH) {
    // Invaid Configuration File Name Length;
    return 0;
  }
          ...
          if (confIsLineASectionName(strippedLineBuffer)) {
            ...
            if (inSection) {
              /* We are so parse the parameter and deal with it
               as appropriate */
              if (confParseParameter(strippedLineBuffer, keyName, keyValue,
                  keyValueSize)) {
...
int getDevices(unsigned int dev_mask[])
{
  ...
  proc = opendir("/proc");
  if (!proc) {
    // No /proc directory or it cannot be opened
    return 1;
  }
  ...
  closedir(proc);
  if (!found) {
    // No running QA devices detected
    return 0;
  }
  return 1;
}
...
int checkLimitDevAccessValue(int * limitDevAccess, char * section_name)
{
  ...
  for (j = 0; j < NUM_DEVICES_TYPES; j++)
    for (i = 0; i < MAX_NUM_DEVICES; i++) {
      if ((devmasks[j] & (1 << i))) {
        sprintf(configFilePath, "/etc/%s_qa_dev%d.conf", dev_names[j], i);
      } else
        continue;
