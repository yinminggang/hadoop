...
public class QatCipher {
  private static final Log LOG =
      LogFactory.getLog(QatCipher.class.getName());
  ...
  /**
   * Priority of the QatCipher shutdown hook.
   */
  ...
  static {
    ...
    try {
      if (!NativeCodeLoader.buildSupportsQat()) {
        PerformanceAdvisory.LOG.debug("Build does not support intel quick assist technology");
        loadingFailure = "build does not support intel quick assist technology.";
      } else {
        initIDs("HADOOP".getBytes("UTF-8"));
      }
    } catch (Throwable t) {
      loadingFailure = t.getMessage();
      LOG.debug("Failed to load Qat Cipher.", t);
    } finally {
  ...
  private QatCipher(long context, int alg, int padding) {
  ...
  /**
   * Return an <code>QatCipher<code> object that implements the specified
   * transformation.
   * 
   * @param transformation the name of the transformation, e.g., 
   * AES/CTR/NoPadding.
   * @return QatCipher an <code>QatCipher<code> object
   * @throws NoSuchAlgorithmException if <code>transformation</code> is null, 
   * empty or in an invalid format.
   * @throws NoSuchPaddingException if <code>transformation</code> contains 
   * a padding scheme that is not available.
   */
  public static final QatCipher getInstance(String transformation) 
    ...
    return new QatCipher(context, algMode, padding);
  }
  ...
  private native static void initIDs(byte[] configSectionName);
  ...
  private native static void stopQat();
